...
void sparoNavicella() {
    for (int i = 0; i < COLONNE; i++) {
        if (tabellone[RIGHE - 1][i] == '^' && tabellone[RIGHE][i] == '-') {
            tabellone[RIGHE][i] = 'p'; //p per proj
        }
    }
}

bool checkProiettileInCampo() {
    ...
    */
    ...
    while (running) {
        ...
        while (SDL_PollEvent(&event)) {
            ...
            /*
            if (event.type == SDL_EVENT_KEY_DOWN && event.key.scancode == SDL_SCANCODE_6) {
                sixPressed = true;
            }
            ...
            if (sixPressed && sevenPressed) {
                sixSeven();
            }
            */

            possibilitàSparare = !checkProiettileInCampo();
            ...
            if (event.type == SDL_EVENT_KEY_DOWN && event.key.scancode == SDL_SCANCODE_SPACE && gioco.stato == 1 ) {
                if (possibilitàSparare) {
                    sparoNavicella();
                }
            }
        ...
        if (gioco.stato == 1) {
            ...
            for (int r = 0; r < 27; r++) {
                for (int c = 0; c < 23; c++) {
                    ...
                    CTabellone[1] = '\0';
                    if (CTabellone[0] == 'X') {
                        EDL_DrawAsset(xscritta,yscritta,nemico, 0, 3.5);
                    }
                    else if (CTabellone[0] == '^') {
                        EDL_DrawAsset(xscritta,yscritta,navicella, 0, 3.5);
                    }
